...
# Parse params according to Config map by Helm chart.
TYPE="${MEMTIER_TYPE}"
...
for I in $(seq 1 $ITERATIONS); do
  ...
  for P in $(seq 1 $BENCHMARKPROCESSES); do
    cat <<EOF | curl --data-binary @- http://pushgateway.monitoring:9091/metrics/job/memtier-benchmark-job/instance/$TYPE

        # TYPE run_information_ gauge
        $(cat /tmp/$I-$P.json | jq -r '."run information" | to_entries | .[] | "run_information_"+ (.key|ascii_downcase|gsub("\\s";"_")) + " " + (.value|tostring)')
    
        # TYPE all_stats_sets_ gauge
        $(cat /tmp/$I-$P.json | jq -r '."ALL STATS".Sets | to_entries | .[] | "all_stats_sets_"+ (.key|ascii_downcase|gsub("/";"_")) + " " + (.value|tostring)')

        # TYPE all_stats_gets_ gauge
        $(cat /tmp/$I-$P.json | jq -r '."ALL STATS".Gets | to_entries | .[] | "all_stats_gets_"+ (.key|ascii_downcase|gsub("/";"_")) + " " + (.value|tostring)')

        # TYPE all_stats_waits_ gauge
        $(cat /tmp/$I-$P.json | jq -r '."ALL STATS".Waits | to_entries | .[] | "all_stats_waits_"+ (.key|ascii_downcase|gsub("/";"_")) + " " + (.value|tostring)')
            
        # TYPE all_stats_totals_ gauge
        $(cat /tmp/$I-$P.json | jq -r '."ALL STATS".Totals | to_entries | .[] | "all_stats_totals_"+ (.key|ascii_downcase|gsub("/";"_")) + " " + (.value|tostring)')
        
        # TYPE request_latency_distribution_set gauge
        $(cat /tmp/$I-$P.json | jq -r '."ALL STATS".SET | .[] | "request_latency_distribution_set{msec=" + "\"" + (."<=msec"|tostring) + "\"" + "}" + " " + (.percent|tostring)')

        # TYPE request_latency_distribution_get gauge
        $(cat /tmp/$I-$P.json | jq -r '."ALL STATS".GET | .[] | "request_latency_distribution_get{msec=" + "\"" + (."<=msec"|tostring) + "\"" + "}" + " " + (.percent|tostring)')
EOF
  done
  output_line "$SUM"
done
